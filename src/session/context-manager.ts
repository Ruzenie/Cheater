...
export class SessionManager {
  ...
  shouldHandoff(): { needed: boolean; reason: string; urgency: string } {
    ...
    switch (status) {
      ...
      case 'critical':
        return { needed: true, reason: `上下文使用 ${this.snapshot.contextUsage.usagePercent}%，已达临界值，应立即迁移`, urgency: 'high' };
      case 'warning':
        return { needed: true, reason: `上下文使用 ${this.snapshot.contextUsage.usagePercent}%，建议准备迁移`, urgency: 'medium' };
      default:
        return { needed: false, reason: `上下文使用 ${this.snapshot.contextUsage.usagePercent}%，状态正常`, urgency: 'none' };
    }
  }
...
${s.completedItems.length > 0
  ? s.completedItems.map((item, i) => `${i + 1}. ✅ ${item.description} → ${item.result} (${item.modelUsed})`).join('\n')
  : '暂无已完成项'}

## 3. 已生成代码
${Object.keys(s.codeArtifacts).length > 0
  ? Object.entries(s.codeArtifacts).map(([file, summary]) => `- \`${file}\`: ${summary}`).join('\n')
  : '暂无代码产出'}

## 4. 关键决策
${s.decisions.length > 0
  ? s.decisions.map((d) => `- **${d.what}**：${d.why}（排除方案：${d.alternatives.join(', ')}）`).join('\n')
  : '暂无关键决策'}
...
${s.nextSteps.length > 0
  ? s.nextSteps.map((step) => `- [${step.priority}] ${step.description}${step.blockedBy ? ` (blocked by: ${step.blockedBy})` : ''}`).join('\n')
  : '无待办'}
...
**指令**：请先用一段话复述"我将做什么/不做什么"，待我确认后再开始执行。`;
  ...
  static loadHandoff(filePath: string): HandoffPackage {
    try {
      ...
      throw new Error(`交接包加载失败 (${filePath}): ${err instanceof Error ? err.message : String(err)}`);
    }
  }
